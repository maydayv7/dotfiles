...
Use [`scripts/pci.sh`](../../scripts/pci.sh) in order to determine the PCI Device IDs which must be added to `virt.vfio.passthrough`
...
The `vfio` and `no-vfio` Specialisations can be selected in the boot menu to toggle GPU passthrough as required
