...
### Credentials
The authentication credentials are managed using [`agenix`](https://github.com/ryantm/agenix) at [`modules/secrets`](../modules/secrets). The encrypted keys are stored at [`modules/secrets/encrypted`](../modules/secrets/encrypted), and a private `git submodule` is present at [`modules/secrets/unencrypted`](../modules/secrets/unencrypted) containing the plain-text credentials. User passwords are made using the command `mkpasswd -m sha-512` and specified using the `passwordFile` option
