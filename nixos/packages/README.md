...
It is advisable to use Flake `inputs` as package sources. To pin it to a specific commit/revision add `?rev=` after the `url`. Else you may also specify the source using `fetch` functions  
...
Otherwise run <code>nix hash convert --hash-algo sha256 $(nix-prefetch-url <b>url</b>)</code> to obtain the hash
