  ...
  perSystem =
    ...
    rec {
      ...
      legacyPackages =
        with inputs;
        let
          nixpkgsConfig = import ./_nixpkgs-config.nix;
        in
        import (config.flake.patchedPkgs system) {
          inherit system;
          config = nixpkgsConfig;

          overlays = (attrValues config.flake.overlays or { }) ++ [
            (_: _: {
    ...
    // (
      let
        call = rec {
          ...
          pkg = name: args: pkgs.callPackage name ({ inherit lib pkgs; } // { util = config.util; } // args);
          script =
            name:
            pkg name {
              inherit inputs;
